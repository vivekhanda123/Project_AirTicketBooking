...
const GetAllFlights = () => {
    ...
    const fetchFlights = async () => {
      try {
        const response = await axios.get(
          "https://localhost:7136/api/Flight/GetAllFlights",
          {
            headers: {
  ...
  const fetchFlightDetails = async (flightId) => {
    try {
      const response = await axios.get(
        `https://localhost:7136/api/Flight/GetFlightDetails/${flightId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const deleteFlight = async (flightId) => {
    try {
      await axios.delete(
        `https://localhost:7136/api/Flight/RemoveFlight/${flightId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
